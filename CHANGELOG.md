...
- 引っ越し先の表示をリモートインスタンスへのリンクボタンではなくインスタンス内のユーザーページの表示にする @atsu1125
- アカウントの引っ越し（リダイレクト設定）機能を追加 @atsu1125
...
- ワードミュート（ハード）の対象となったノートを確認できるようにする @atsu1125
- Node 18からNode 22にアップデート @atsu1125
...
- 非ログイン状態でリノートしたユーザーのリストが見れないのを修正 @atsu1125
- 凍結しているリモートユーザーについてUpdate, Undo, Reject, and Deleteのアクティビティは許可するように @atsu1125
- 正常終了したジョブキューが削除されないのを修正 @syuilo @Johann150
...
- リモートユーザーのブロック時にエラーとなりアクティビティが送信されない問題を修正 @xianonn @atsu1125
...
- プロフィールを隠す機能は表示対象をローカルフォロワーのみではなくローカルのみにする @atsu1125
- ローカルユーザーを凍結しリモートにアカウント削除を配信する前にローカル→リモートとリモート→ローカルのフォローは解除する @atsu1125
- リモートのアカウント削除が取り消されたら取り消すのをやめる @atsu1125

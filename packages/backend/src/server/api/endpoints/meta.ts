...
export default define(meta, paramDef, async (ps, me, token) => {
	...
	if (ps.detail) {
		...
		if (me && me.isAdmin && !token) {
			...
			response.sensitiveWords = instance.sensitiveWords;
			...
			response.doNotSendNotificationEmailsForAbuseReportToModerator = instance.doNotSendNotificationEmailsForAbuseReportToModerator;
